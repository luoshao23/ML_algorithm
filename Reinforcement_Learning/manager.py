...
class Manager:
    def __init__(self,
        ...
        self.grid_h, self.grid_w = env_info["grid_height"], env_info[
            "grid_width"]
    ...
    def compute_values_table(self, values):
        ...
        for state in range(len(values)):
            self.values_table[np.unravel_index(
                state, (self.grid_h, self.grid_w))] = values[state]
    ...
    def visualize(self, values, episode_num):
        ...
        plt.xticks([])
        plt.yticks([])
        im = plt.imshow(self.values_table,
                        cmap=self.cmap,
        ...
        if self.true_values is not None:
            ...
            plt.xticks([])
            plt.yticks([])
            im = plt.imshow((self.values_table - self.true_values)**2,
                            origin='upper',
                            cmap=self.cmap_VE)
            ...
            plt.subplot(313)
            plt.plot([x[0] for x in self.RMSVE_LOG],
                     [x[1] for x in self.RMSVE_LOG])
            plt.xlabel("Episode")
    ...
    def run_tests(self, values, RMSVE_threshold):
        ...
        if self.compute_RMSVE() < RMSVE_threshold and np.allclose(
                self.true_values[mask], self.values_table[mask]):
            pass
        else:
            assert False
