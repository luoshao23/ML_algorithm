...
def schedulecost(sol):
    ...
    for d in xrange(len(sol)/2):
        ...
        if latestarrival < getminutes(out[1]):
            latestarrival = getminutes(out[1])
...
def annealingopt(domain, costf=schedulecost, T=10000.0, cool=0.95, step=1):
    ...
    sol = []
    for k in xrange(100):
        tmpsol = [random.randint(domain[i][0], domain[i][1])
                  for i in xrange(len(domain))]
    ...
    while T > 0.1:
        ...
        if solb[i] > domain[i][1]:
            solb[i] = domain[i][1]
        elif solb[i] < domain[i][0]:
            solb[i] = domain[i][0]
